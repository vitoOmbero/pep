...
void Game::LoadAssets(Culture::Language lang) {
    lang_ = lang;
    title_ = GuessNumberAssets::game_description_strings.at(lang).title;
    default_filler = GuessNumberAssets::game_description_strings.at(lang).default_filler.at(0);
}
...
void Game::Play() const {
    ...
    oss << GuessNumberAssets::game_stage_strings.at(getLang())[0] << getTitle() << "!";
    ...
    Terminal::InputValue(
            GuessNumberAssets::game_stage_strings.at(getLang())[1] ,
    ...
    oss.str("");
    oss.clear();
    oss << GuessNumberAssets::game_stage_strings.at(getLang())[6] << std::to_string(secret_number);
    ...
    while (is_not_finished) {
        size_t num;
        Terminal::InputValue(GuessNumberAssets::game_stage_strings.at(getLang())[7],
                             GuessNumberAssets::game_stage_strings.at(getLang())[8],
        ...
        if (num == secret_number) {
            Terminal::ReportVal(GuessNumberAssets::game_stage_strings.at(getLang())[11]
                                , num, "!");
