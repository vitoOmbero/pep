...
void Terminal::ReportVal(std::string_view msg, size_t val, std::string_view msg2) {
    std::cout << msg << val << msg2 << std::endl;
}

void Terminal::InputValue(std::string_view msg_tip, std::string_view msg_fail, size_t &val_storage, size_t min,
                          size_t max) {
    int tries = 0;
    while (true) {
...
void Terminal::ReportTip(std::string_view tip) {
    std::cout << tip;
}
