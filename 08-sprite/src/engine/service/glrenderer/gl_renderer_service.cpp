...
void Load(visual_world_objects::Type vwo) {
  ...
  if (std::get_if<visual_world_objects::SpriteQuad>(&vwo) != nullptr) {
    ...
    if (!vrt.mesh.getTexture().texture_asset_path.empty()) {
#ifdef PEP_DEBUG
      Terminal::ReportErr("Object has src for texture. Texture is ignored.");
#endif
    } else {
      const std::vector<std::string>& sprite_multi_image =
          gGameConfiguration
              .SpriteMultiImages->at(vrt.game_sprite_multi_image_index);

      // load sprite multi image

      SpriteImage si;
      stbi_set_flip_vertically_on_load(GL_TRUE);
      std::for_each(sprite_multi_image.cbegin(), sprite_multi_image.cend(),
                    [&si](const std::string& path) {
                      int width, height, components;
                      unsigned char* imageData =
                          stbi_load(path.data(), &width, &height, &components,
                                    STBI_rgb_alpha);
                      if (imageData == nullptr) {
                        Terminal::ReportMsg(
                            ">> GlRendererService:"
                            "Error loading texture :");
                        Terminal::ReportMsg(path);
                      }
...
#ifdef PEP_DEBUG
      Terminal::ReportErr(
          "Object has wrong RenderingTargetType! Default is used.");
#endif
    }
    visual_objects.emplace_back(vrt);
  }
