...
void GlRenderer::ProcessMeshBoundedRenderingTargets(
  ...
  // TODO: refactor this (and whole the rest code)
...
void GlRenderer::LoadSprite(const visual_world_objects::Type &vwo) {
  auto &vrt = std::get<visual_world_objects::SpriteQuad>(vwo);

  if (!vrt.sprite_image.IsMultiImage()) {
#ifdef PEP_DEBUG
    Terminal::ReportErr("Not multi image sprites are not currently supported.");
#endif
    return;
  }

  auto current_ebo = ebo_[ebo_.size() - 1];

  // TODO: remove static sprite speed & add sprite animation player
  i_sprite_target_frame_time_[current_ebo] = std::chrono::milliseconds(250);
  i_sprite_target_frame_index_[current_ebo] = 0;

  // NOTE: code duplication - see LoadTexture

  for (size_t i = 0; i < vrt.sprite_image.size() - 1; ++i) {
    GLuint texture;
    glGenTextures(1, &texture);
    i_target_textures_.insert({ebo_[ebo_.size() - 1], texture});
    glBindTexture(GL_TEXTURE_2D, texture);
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, vrt.sprite_image.getWidth(),
                 vrt.sprite_image.getHeight(), 0, GL_RGBA, GL_UNSIGNED_BYTE,
                 vrt.sprite_image.at(i));

    glGenerateMipmap(GL_TEXTURE_2D);

    // TODO: image asset managing system
    // stbi_image_free(imageData);
    glBindTexture(GL_TEXTURE_2D, 0);
  }
}
...
void GlRenderer::RenderFrame() {
  ...
  if (!ebo_.empty()) {
    ...
    for (size_t i = 0; i < ebo_.size(); ++i) {
      ...
      // bind textures
      if (i_target_textures_.count(ebo_[i]) == 1) {
        auto range = i_target_textures_.equal_range(ebo_[i]);
        assert(texture_bind_counter >= 0 &&
               texture_bind_counter < 80);  // see khronos docs
        glActiveTexture(GL_TEXTURE0 + texture_bind_counter);
        glBindTexture(GL_TEXTURE_2D, range.first->second);
        /*        using by default
                  glUniform1i(glGetUniformLocation(i_target_shaders_[i],
           "texture_s1"), 0);
        */
        ++texture_bind_counter;
      }
      if (i_target_textures_.count(ebo_[i]) > 1) {
        // draw current sprite frame
        auto &current_frame_index = i_sprite_target_frame_index_[ebo_[i]];
        ...
        for (auto j = range.first; j != range.second; ++j) {
          if (index == current_frame_index) texture_gl_id = j->second;
          ++index;
        }
        ...
        */
        ...
        // set new current sprite frame
        auto image_frame_showtime = i_sprite_target_frame_time_[ebo_[i]];
        static Uint32 tp_last_showtime = UINT32_MAX;
        auto tp_current_showtime = SDL_GetTicks();
        if (tp_current_showtime - tp_last_showtime > image_frame_showtime.count()) {
          ++current_frame_index;
          if (current_frame_index >= std::distance(range.first, range.second))
            current_frame_index = 0;
          tp_last_showtime = tp_current_showtime;
        }
