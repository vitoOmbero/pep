#include "service/text_world/text_world_object_processor.h"
...
void TextWorldObjectProcessor::Process(text_world_objects::Type two) const {
  auto level = (*gGameConfiguration.Levels).at(current_level_id_);
  ...
  if (std::get_if<text_world_objects::Stand>(&two) != nullptr) {
    ...
    std::string message =
        gGameConfiguration.StringAssets->at(gGameConfiguration.Language)
            .at(asset_id);
    ...
    if (stand.is_input_expected) {
      Terminal::PressAnyKeyToContinue(
          Culture::strings.at(gGameConfiguration.Language)
              .description_strings.input_tip);
    }
  }
