...
void GameLevelLogicProcessor::ProcessLevel(size_t level_id) {
  ...
  (*Game::Levels)[level_id].game_logic.OnLoad();
  (*Game::Levels)[level_id].game_logic.Body();
  (*Game::Levels)[level_id].game_logic.Transition();
}
...
void GameLevelLogicProcessor::LoadTextWorldObject(size_t level_based_two_id) {
  auto two = (*Game::Levels)[current_processing_level_id_]
