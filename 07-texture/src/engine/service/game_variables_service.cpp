...
VariableType GameVariablesService::DefineType(game_var_name var) {
  auto it =
      std::find_if(gGameConfiguration.GlobalVariableDeclarations->cbegin(),
                   gGameConfiguration.GlobalVariableDeclarations->cend(),
                   [&var](auto it) { return it.name == var; });

#ifdef PEP_VERBOSE_GAME_LOGIC
  if (it == gGameConfiguration.GlobalVariableDeclarations->cend()) {
    Terminal::ReportErr(
